...
plot <- ggplot(data = main_df, mapping = aes(x = cag_diff,
  ...
  xlab("Average Treatment Effect (GCSE Grade Points)") + ylab("Category Value") +
  # Retitle legend
...
two_way_vars %>% str_to_lower() %>% str_replace_all(" x\n", "_")
...
for (factor_var in two_way_vars){
  ...
  plot <- ggplot(data = subdf, mapping = aes(x = cag_diff,
    ...
    geom_point(size = 3) + 
    # Add titles and labels
    ggtitle(glue("ATE | ", clean_name)) +
    xlab("Average Treatment Effect (GCSE Grade Points)") +
    ylab(clean_name) +
    # Retitle legend
